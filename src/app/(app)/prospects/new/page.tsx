...
export default function NewProspectPage() {
  ...
  const [form, setForm] = useState({
    ...
    source: 'other', lead_score: '0', notes: '',
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      if (res.status === 409) {
        alert('A company with that Companies House number already exists.');
      }
      setSaving(false);
    }
  }
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        <div className="flex items-center gap-2 text-xs text-slate-500 mb-1">
          <Link href="/prospects" className="hover:text-slate-300">Prospects</Link>
          <span>/</span>
          <span>New Prospect</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-100">Add Prospect</h1>
        <p className="text-sm text-slate-500 mt-0.5">Prospects can later be converted into active leads.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <input
              type="text" value={form.company_number}
          ...
          <div className="space-y-1">
            ...
            <input
              type="date" value={form.incorporated}
              onChange={e => set('incorporated', e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            ...
            <input
              type="text" value={form.sic_label}
              onChange={e => set('sic_label', e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-slate-400">Score (0–100)</label>
            <input
              type="number" min="0" max="100" value={form.lead_score}
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="submit" disabled={saving || !form.company_name.trim()}
          ...
          <Link href="/prospects" className="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium rounded-lg transition-colors">
            Cancel
          </Link>
        </div>
      </form>
